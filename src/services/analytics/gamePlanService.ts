...
class GamePlanService {
  ...
  calculateGamePlan(
    ...
    const daysRemainingInMonth = Math.max(1, Math.ceil((monthEnd.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)));
    ...
    const mtdCommissionsData = commissions.filter((c) => {
      ...
      return createdDate >= monthStart && createdDate <= monthEnd && c.status === 'paid';
    });

    // Calculate MTD commissions earned
    const mtdCommissions = mtdCommissionsData.reduce((sum, c) => sum + (c.amount || 0), 0);

    // Calculate gap (how much more commission needed)
    const gap = Math.max(0, grossCommissionNeeded - mtdCommissions);

    // Calculate progress percentage
    const progressPercent = grossCommissionNeeded > 0 ? (mtdCommissions / grossCommissionNeeded) * 100 : 0;
    ...
    const avgPolicyCommission = mtdPolicies.length > 0
      ? mtdCommissions / mtdPolicies.length
      : 800; // Fallback estimate if no policies yet
    ...
    const pathOptions = this.generatePathOptions(gap, daysRemainingInMonth, avgPolicyCommission);
  ...
  private generateSmartMoves(
    ...
    const productCounts = mtdPolicies.reduce((acc, p) => {
      const product = p.product || 'Unknown';
      acc[product] = (acc[product] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const bestProduct = Object.entries(productCounts)
      .sort(([, a], [, b]) => b - a)[0];
  ...
  private generateScenarios(
    ...
    const projectedFromCurrentPace = currentPoliciesPerWeek * weeksRemaining * avgCommission;
    const currentPaceTotal = mtdCommissions + projectedFromCurrentPace;

    // Scenario 2: Add 1 policy per week
    const projectedPlus1 = (currentPoliciesPerWeek + 1) * weeksRemaining * avgCommission;
    const plus1Total = mtdCommissions + projectedPlus1;

    // Scenario 3: Add 2 policies per week
    const projectedPlus2 = (currentPoliciesPerWeek + 2) * weeksRemaining * avgCommission;
  ...
  private findBestProduct(
    ...
    // Group by product
    const productData = mtdPolicies.reduce((acc, p) => {
      const product = p.product || 'Unknown';
      if (!acc[product]) {
        acc[product] = { count: 0 };
      }
    ...
    const totalCommission = mtdCommissions.reduce((sum, c) => sum + (c.amount || 0), 0);
    const avgCommission = mtdPolicies.length > 0 ? totalCommission / mtdPolicies.length : 0;

    // Find product with most sales
    const best = Object.entries(productData)
