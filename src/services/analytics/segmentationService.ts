 ...
 */
export function segmentClientsByValue(policies: Policy[]): ClientSegmentationSummary {
  ...
  clientMap.forEach((clientPolicies, clientId) => {
    ...
    const totalPremium = clientPolicies.reduce((sum, p) => sum + (p.annualPremium || 0), 0);
    const avgPremium = totalPolicies > 0 ? totalPremium / totalPolicies : 0;

    // Get unique states and products
    const states = [...new Set(clientPolicies.map(p => p.client?.state).filter(Boolean))];
    ...
    const firstPolicyDate = new Date(Math.min(...policyDates.map(d => d.getTime())));
    const latestPolicyDate = new Date(Math.max(...policyDates.map(d => d.getTime())));
    const monthsAsClient = Math.floor(
      (new Date().getTime() - firstPolicyDate.getTime()) / (1000 * 60 * 60 * 24 * 30)
    );
  ...
  const avgPremiumByTier: Record<ClientValueTier, number> = {
    high: highValue.reduce((sum, c) => sum + c.avgPremium, 0) / (highValue.length || 1),
    medium: mediumValue.reduce((sum, c) => sum + c.avgPremium, 0) / (mediumValue.length || 1),
    low: lowValue.reduce((sum, c) => sum + c.avgPremium, 0) / (lowValue.length || 1),
  };
 ...
 */
export function calculateCrossSellOpportunities(policies: Policy[]): CrossSellOpportunity[] {
  const allProducts = ['whole_life', 'term', 'universal_life', 'indexed_universal_life', 'accidental', 'final_expense', 'annuity'];
  ...
  clientMap.forEach((clientPolicies, clientId) => {
    ...
    const missingProducts = allProducts.filter(p => !currentProducts.includes(p as any));

    // Only consider if client has less than all products
    if (missingProducts.length > 0) {
      const totalPremium = clientPolicies.reduce((sum, p) => sum + (p.annualPremium || 0), 0);
      const avgPremium = clientPolicies.length > 0 ? totalPremium / clientPolicies.length : 0;
      ...
      const tenureMonths = Math.floor(
        (new Date().getTime() - new Date(clientPolicies[0].effectiveDate).getTime()) /
        (1000 * 60 * 60 * 24 * 30)
      );
      const tenureScore = Math.min(20, (tenureMonths / 12) * 20); // Max 20 points for 1+ year

      const opportunityScore = Math.min(100, Math.round(valueScore + productScore + tenureScore));

      // Recommend products based on current holdings
      const recommendedProducts = getRecommendedProducts(currentProducts, missingProducts);
 ...
 */
export function getClientLifetimeValue(policies: Policy[]): ClientLifetimeValue[] {
  // Group policies by client
  const clientMap = new Map<string, Policy[]>();
  ...
  clientMap.forEach((clientPolicies, clientId) => {
    ...
    const lifetimeValue = clientPolicies.reduce((sum, p) => sum + (p.annualPremium || 0), 0);
    const avgPolicyValue = totalPolicies > 0 ? lifetimeValue / totalPolicies : 0;
    ...
    const estimatedFutureValue = retentionRate > 50
    ...
    const lapsedPolicies = clientPolicies.filter(p => p.status === 'lapsed').length;
    const cancelledPolicies = clientPolicies.filter(p => p.status === 'cancelled').length;
    const riskScore = totalPolicies > 0
      ? Math.round(((lapsedPolicies + cancelledPolicies) / totalPolicies) * 100)
      : 0;
 ...
 */
function getRecommendedProducts(currentProducts: string[], missingProducts: string[]): string[] {
  ...
  if (currentProducts.includes('term') && missingProducts.includes('whole_life')) {
    recommendations.push('whole_life');
  }

  if (currentProducts.includes('whole_life') && missingProducts.includes('annuity')) {
