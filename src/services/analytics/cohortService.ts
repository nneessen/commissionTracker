...
export function getCohortRetention(policies: Policy[]): CohortRetentionData[] {
  ...
  policies.forEach((policy) => {
    const cohortMonth = format(parseLocalDate(policy.effectiveDate), "yyyy-MM");
  ...
  cohortMap.forEach((cohortPolicies, cohortMonth) => {
    ...
    for (
      let monthsElapsed = 0;
      monthsElapsed <= Math.min(maxMonths, 24);
      monthsElapsed++
    ) {
      ...
      retentionByMonth[monthsElapsed] =
        totalPolicies > 0 ? (active / totalPolicies) * 100 : 0;
    }
...
export function getChargebacksByCohort(
  ...
  cohortMap.forEach((cohortInfo, cohortMonth) => {
    ...
    const totalCommission = cohortInfo.commissions.reduce(
      (sum, c) => sum + (c.amount || 0),
      0,
    );
    const chargebackAmount = cohortInfo.chargebacks.reduce(
      (sum, c) => sum + (c.chargebackAmount || 0),
      0,
    );
    const chargebackRate =
      totalCommission > 0 ? (chargebackAmount / totalCommission) * 100 : 0;
    ...
    cohortInfo.chargebacks.forEach((commission) => {
      ...
      if (policy && commission.chargebackDate) {
        const monthsToChargeback = differenceInMonths(
          ...
          parseLocalDate(policy.effectiveDate),
        );
        totalMonthsToChargeback += monthsToChargeback;
        chargebacksByMonth[monthsToChargeback] =
          (chargebacksByMonth[monthsToChargeback] || 0) + 1;
      }
    });

    const avgMonthsToChargeback =
      cohortInfo.chargebacks.length > 0
...
export function getEarningProgressByCohort(
  ...
  cohortMap.forEach((cohortCommissions, cohortMonth) => {
    const cohortDate = parseISO(`${cohortMonth}-01`);
    const totalAdvance = cohortCommissions.reduce(
      (sum, c) => sum + (c.amount || 0),
      0,
    );
    const totalEarned = cohortCommissions.reduce(
      (sum, c) => sum + (c.earnedAmount || 0),
      0,
    );
    const totalUnearned = cohortCommissions.reduce(
      (sum, c) => sum + (c.unearnedAmount || 0),
      0,
    );
    const earningRate =
      totalAdvance > 0 ? (totalEarned / totalAdvance) * 100 : 0;
    ...
    for (
      ...
      progressByMonth[monthsElapsed] =
        totalAdvance > 0 ? (earnedAtMonth / totalAdvance) * 100 : 0;
    }
...
export function getCohortSummary(
  ...
  const avgRetention9Month =
    retentionData
      .filter((c) => c.retentionByMonth[9] !== undefined)
      .reduce((sum, c) => sum + c.retentionByMonth[9], 0) /
    (retentionData.filter((c) => c.retentionByMonth[9] !== undefined).length ||
      1);

  const avgChargebackRate =
    chargebackData.reduce((sum, c) => sum + c.chargebackRate, 0) /
    (chargebackData.length || 1);

  const avgEarningRate =
    earningData.reduce((sum, c) => sum + c.earningRate, 0) /
  ...
  return {
    ...
    avgChargebackRate,
    avgEarningRate,
    bestCohort: retentionData.sort(
      (a, b) => (b.retentionByMonth[9] || 0) - (a.retentionByMonth[9] || 0),
    )[0]?.cohortLabel,
    worstCohort: retentionData.sort(
      (a, b) => (a.retentionByMonth[9] || 0) - (b.retentionByMonth[9] || 0),
    )[0]?.cohortLabel,
  };
}
