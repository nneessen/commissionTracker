...
export function calculateContribution(
  ...
  const previousCommission = previousPeriodCommissions.reduce(
    (sum, c) => sum + (c.amount || 0),
    0,
  );
  // Commission rate is already stored as decimal (0.95 for 95%) in database
  ...
  const volumeEffect =
    (currentPolicies - previousPolicies) * previousAvgPremium * previousAvgRate;

  // Rate effect: Change in commission rate (holding volume and premium constant)
  const rateEffect =
    currentPolicies * previousAvgPremium * (currentAvgRate - previousAvgRate);

  // Mix effect: Change in average premium (product mix shift)
  const mixEffect =
    currentPolicies * (currentAvgPremium - previousAvgPremium) * currentAvgRate;
  ...
  const volumePercent =
    totalChange !== 0 ? (volumeEffect / Math.abs(totalChange)) * 100 : 0;
  const ratePercent =
    totalChange !== 0 ? (rateEffect / Math.abs(totalChange)) * 100 : 0;
  const mixPercent =
    totalChange !== 0 ? (mixEffect / Math.abs(totalChange)) * 100 : 0;
 ...
 */
export function getProductMixEvolution(
  policies: Policy[],
): ProductMixEvolution[] {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const totalRevenue = monthPolicies.reduce(
      (sum, p) => sum + (p.annualPremium || 0),
      0,
    );
    ...
    monthPolicies.forEach((policy) => {
      const existing = productMap.get(policy.product) || {
        count: 0,
        revenue: 0,
      };
...
export function getTopMovers(
  ...
  currentPeriodPolicies.forEach((p) => {
    ...
    const commission = currentPeriodCommissions.find(
      (c) => c.policyId === p.id,
    );
    if (commission) {
  ...
  previousPeriodPolicies.forEach((p) => {
    const state = p.client?.state || "Unknown";
    const commission = previousPeriodCommissions.find(
      (c) => c.policyId === p.id,
    );
    if (commission) {
