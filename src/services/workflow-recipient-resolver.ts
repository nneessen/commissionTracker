...
export async function resolveRecipients(
  ...
  try {
    switch (config.type) {
      ...
      // Pipeline
      case "pipeline_phase":
        result = await resolvePipelinePhase(
          config.phaseIds || [],
          config.phaseStatuses,
          maxRecipients,
        );
        break;
...
async function resolveDirectUpline(
  context: ResolverContext,
): Promise<ResolvedRecipients> {
...
async function resolveDirectDownline(
  context: ResolverContext,
  max: number,
): Promise<ResolvedRecipients> {
  const userId = context.recipientId || context.triggeredBy;
...
async function resolveEntireDownline(
  context: ResolverContext,
  max: number,
): Promise<ResolvedRecipients> {
  const userId = context.recipientId || context.triggeredBy;
...
async function resolvePolicyAgent(
  context: ResolverContext,
): Promise<ResolvedRecipients> {
...
async function resolvePolicyClient(
  context: ResolverContext,
): Promise<ResolvedRecipients> {
  if (!context.policyId) return emptyResult();
...
async function resolveCommissionRecipient(
  context: ResolverContext,
): Promise<ResolvedRecipients> {
...
async function resolvePipelineRecruiter(
  context: ResolverContext,
): Promise<ResolvedRecipients> {
  const userId = context.recipientId || context.triggeredBy;
  if (!userId) return emptyResult();
...
async function resolvePipelineUpline(
  context: ResolverContext,
): Promise<ResolvedRecipients> {
