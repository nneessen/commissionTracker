...
describe('CompGuideService - New Methods', () => {
  ...
  describe('getEntriesByProduct', () => {
    it('should fetch comp rates for a specific product', async () => {
      ...
      expect(mockOrder).toHaveBeenCalledWith('contract_level', { ascending: true });
  ...
  describe('createBulkEntries', () => {
    it('should create multiple comp guide entries', async () => {
      ...
      const result = await compGuideService.createBulkEntries(newEntries as any);
    ...
    it('should handle all 14 contract levels for a product', async () => {
      const productId = 'p1';
      const carrierId = 'c1';
      const contractLevels = [80, 85, 90, 95, 100, 105, 110, 115, 120, 125, 130, 135, 140, 145];

      const newEntries = contractLevels.map((level) => ({
        product_id: productId,
        carrier_id: carrierId,
        contract_level: level,
        commission_percentage: 0.70 + (level - 80) * 0.005,
        effective_date: '2025-01-01',
      }));
      ...
      const levels = result.data?.map(entry => entry.contract_level);
  ...
  describe('getEntriesByCarrier (existing method, verify ordering)', () => {
    it('should order by contract_level ascending', async () => {
      ...
      const mockRates = [
        { id: '1', carrier_id: carrierId, contract_level: 80, commission_percentage: 0.70 },
        { id: '2', carrier_id: carrierId, contract_level: 85, commission_percentage: 0.75 },
        { id: '3', carrier_id: carrierId, contract_level: 90, commission_percentage: 0.80 },
      ];
