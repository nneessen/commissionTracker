...
class HierarchyService {
  ...
  async getMyHierarchyTree(): Promise<HierarchyNode[]> {
    try {
      ...
      const overridesByAgent = (overrides || []).reduce((acc, o) => {
  ...
  async getAllDownlinePerformance(): Promise<DownlinePerformance[]> {
    try {
      ...
      if (policiesError) {
        throw new DatabaseError('getAllDownlinePerformance.policies', policiesError);
      }
      ...
      if (commissionsError) {
        throw new DatabaseError('getAllDownlinePerformance.commissions', commissionsError);
      }
      ...
      if (overridesError) {
        throw new DatabaseError('getAllDownlinePerformance.overrides', overridesError);
      }

      // Aggregate data by downline
      return downlines.map(downline => {
        const downlinePolicies = (policies || []).filter(p => p.user_id === downline.id);
        const downlineCommissions = (commissions || []).filter(c => c.user_id === downline.id);
        const downlineOverrides = (overrides || []).filter(o => o.base_agent_id === downline.id);
      ...
      logger.error('HierarchyService.getAllDownlinePerformance', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async getAgentDetails(agentId: string): Promise<any> {
    try {
      ...
      const persistencyRate = policyMetrics.totalPolicies > 0
      ...
      return {
        ...
        avgPremium: policyMetrics.activePolicies > 0
      ...
      logger.error('HierarchyService.getAgentDetails', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
    ...
    try {
      ...
      return {
        ...
        policies: (policies || []).map(p => ({
          ...
          status: p.status,
          issueDate: p.effective_date || p.created_at || new Date().toISOString(),
        }))
      };
    } catch (error) {
      logger.error('HierarchyService.getAgentPolicies', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async getAgentCommissions(agentId: string): Promise<any> {
    try {
      ...
      logger.error('HierarchyService.getAgentCommissions', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async getAgentOverrides(agentId: string): Promise<any> {
    try {
      ...
      logger.error('HierarchyService.getAgentOverrides', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async getTeamComparison(agentId: string): Promise<any> {
    try {
      ...
      const peerPerformance = peerIds.map(peerId => {
        const agentPolicies = (peerPolicies || []).filter(p => p.user_id === peerId);
        const active = agentPolicies.filter(p => p.status === 'active');
        const totalPremium = active.reduce((sum, p) => sum + parseFloat(String(p.annual_premium) || '0'), 0);
        const persistency = agentPolicies.length > 0
      ...
      const percentile = totalAgents > 0 ? Math.round(((totalAgents - agentRank + 1) / totalAgents) * 100) : 0;

      // Calculate averages
      const avgPremium = peerPerformance.reduce((sum, p) => sum + p.premium, 0) / totalAgents;
      const avgPolicies = peerPerformance.reduce((sum, p) => sum + p.policies, 0) / totalAgents;
      const avgPersistency = peerPerformance.reduce((sum, p) => sum + p.persistency, 0) / totalAgents;
      ...
      logger.error('HierarchyService.getTeamComparison', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
   ...
   * @private
   */
  private buildTree(profiles: UserProfile[], rootId: string, overridesByAgent?: Record<string, number>): HierarchyNode[] {
