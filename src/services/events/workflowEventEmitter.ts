...
class WorkflowEventEmitter {
   ...
   * @param eventName The name of the event (e.g., 'recruit.graduated_to_agent')
   * @param context The context data associated with the event
   */
  async emit(eventName: string, context: EventContext): Promise<EventEmissionResult> {
    ...
    try {
      ...
      // Find all active workflows that listen to this event
      const workflows = await this.findTriggeredWorkflows(eventName);

      if (workflows.length === 0) {
        console.log(`[EventEmitter] No active workflows found for event: ${eventName}`);
        return result;
      }

      console.log(`[EventEmitter] Found ${workflows.length} workflows for event: ${eventName}`);
   ...
   */
  private async recordEvent(eventName: string, context: EventContext): Promise<void> {
    try {
      await supabase
  ...
  private async executeWorkflow(
    ...
    console.log(`[EventEmitter] Executing workflow: ${workflow.name} for event: ${eventName}`);

    // Check cooldown period
    const canRun = await this.checkWorkflowCooldown(workflow, context);
    if (!canRun) {
      console.log(`[EventEmitter] Workflow ${workflow.name} is in cooldown period`);
      return;
    }

    // Check conditions if any
    if (!this.evaluateConditions(workflow.conditions, context)) {
      console.log(`[EventEmitter] Workflow ${workflow.name} conditions not met`);
      return;
    }
    ...
    supabase.functions.invoke('process-workflow', {
      ...
      if (response.error) {
        console.error(`[EventEmitter] Workflow processor error for ${workflow.name}:`, response.error);
      } else {
        console.log(`[EventEmitter] Workflow ${workflow.name} triggered successfully`);
      }
    }).catch((err) => {
      console.error(`[EventEmitter] Failed to invoke workflow processor:`, err);
    });
  }
  ...
  private async checkWorkflowCooldown(
    ...
    cooldownTime.setMinutes(cooldownTime.getMinutes() - workflow.cooldownMinutes);
  ...
  private evaluateConditions(
    ...
    for (const condition of conditions) {
      ...
      const conditionMet = this.evaluateCondition(fieldValue, cond.operator, cond.value);
   ...
   */
  async emitBatch(events: Array<{ eventName: string; context: EventContext }>): Promise<void> {
