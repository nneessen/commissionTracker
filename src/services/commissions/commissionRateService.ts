...
class CommissionRateService {
  ...
  async getByProduct(productId: string): Promise<CommissionRate[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch commission rates for product: ${error.message}`,
      );
    }
  ...
  async getByCarrier(carrierId: string): Promise<CommissionRate[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch commission rates for carrier: ${error.message}`,
      );
    }
  ...
  async getByCarrierAndProduct(
    ...
    if (error) {
      throw new Error(
        `Failed to fetch commission rates for carrier and product: ${error.message}`,
      );
    }
  ...
  async getByContractLevel(contractLevel: number): Promise<CommissionRate[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch commission rates for contract level: ${error.message}`,
      );
    }
  ...
  async createBulk(
    ratesData: CreateCommissionRateData[],
  ): Promise<CommissionRate[]> {
  ...
  async update(
    id: string,
    updates: Partial<CreateCommissionRateData>,
  ): Promise<CommissionRate> {
  ...
  async deleteByProduct(productId: string): Promise<void> {
    ...
    if (error) {
      throw new Error(
        `Failed to delete commission rates for product: ${error.message}`,
      );
    }
  }
  ...
  async deleteByCarrier(carrierId: string): Promise<void> {
    ...
    if (error) {
      throw new Error(
        `Failed to delete commission rates for carrier: ${error.message}`,
      );
    }
  }
  ...
  private transformToDB(data: Partial<CreateCommissionRateData>): any {
    ...
    if (data.contractLevel !== undefined)
      dbData.contract_level = data.contractLevel;
  ...
  async getUserCommissionProfile(
    ...
    try {
      ...
      if (error) {
        logger.error("Failed to get user commission profile", {
          error,
          userId,
          lookbackMonths,
        });
        throw new Error(
          `Failed to calculate commission profile: ${error.message}`,
        );
      }
      ...
      logger.error("Error in getUserCommissionProfile", {
        error,
        userId,
        lookbackMonths,
      });
