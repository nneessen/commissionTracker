...
export function useMetrics() {
  ...
  const clientMetrics = useMemo((): ClientMetrics => {
    ...
    const averageAge = policies.length > 0
    ...
    const totalLifetimeValue = Array.from(clientValues.values()).reduce((sum, val) => sum + val, 0);
    const averageLifetimeValue = clientValues.size > 0 ? totalLifetimeValue / clientValues.size : 0;
    const averagePoliciesPerClient = totalClients > 0 ? policies.length / totalClients : 0;
  ...
  const policyMetrics = useMemo((): PolicyMetrics => {
    ...
    const lapsedRate = totalPolicies > 0
      ? (lapsedPolicies / totalPolicies) * 100
      : 0;

    const cancellationRate = totalPolicies > 0
      ? (cancelledPolicies / totalPolicies) * 100
      : 0;
    ...
    // Premium by product
    const premiumByProduct = policies.reduce((acc, p) => {
    ...
    policies.forEach(p => {
      carrierDistribution.set(p.carrierId, (carrierDistribution.get(p.carrierId) || 0) + 1);
    });

    const policyDistributionByCarrier = Array.from(carrierDistribution.entries())
      .map(([carrierId, count]) => ({
        carrierId,
    ...
    const monthlyNewPolicies: Array<{ month: string; count: number; premium: number }> = [];
    const now = new Date();
    for (let i = 11; i >= 0; i--) {
    ...
    const averageTermLength = policiesWithTerm.length > 0
      ? policiesWithTerm.reduce((sum, p) => sum + (p.termLength || 0), 0) / policiesWithTerm.length
      : 0;
    ...
    const policiesExpiringThisMonth = policies.filter(p => {
      ...
      const expiryDate = new Date(p.effectiveDate);
  ...
  const commissionMetrics = useMemo((): CommissionMetrics => {
    ...
    const averageCommissionPerPolicy = policies.length > 0
      ? totalEarned / policies.length
      : 0;
    ...
    commissions.forEach(c => {
      carrierCommissions.set(c.carrierId, (carrierCommissions.get(c.carrierId) || 0) + c.commissionAmount);
    });
    ...
    const productCommissions = new Map<ProductType, { amount: number; count: number }>();
    commissions.forEach(c => {
      const current = productCommissions.get(c.product) || { amount: 0, count: 0 };
      productCommissions.set(c.product, {
        amount: current.amount + c.commissionAmount,
    ...
    // Commission trends
    const commissionTrends: Array<{ period: string; amount: number; policyCount: number }> = [];
    for (let i = 11; i >= 0; i--) {
      const monthStart = new Date(now.getFullYear(), now.getMonth() - i, 1);
      const monthEnd = new Date(now.getFullYear(), now.getMonth() - i + 1, 0);
      ...
      commissionTrends.push({
        ...
        amount: monthCommissions.reduce((sum, c) => sum + c.commissionAmount, 0),
    ...
    const projectedAnnualCommission = monthsElapsed > 0
      ? (thisYearEarnings / monthsElapsed) * 12
      : 0;
  ...
  const productPerformance = useMemo((): ProductPerformance[] => {
    ...
    return products.map(product => {
      ...
      const totalPremium = productPolicies.reduce((sum, p) => sum + p.annualPremium, 0);
      const averagePremium = productPolicies.length > 0 ? totalPremium / productPolicies.length : 0;

      // Calculate revenue from commissions for this product
      const productCommissions = commissions.filter(c => c.product === product);
      const totalRevenue = productCommissions.reduce((sum, c) => sum + c.commissionAmount, 0);
  ...
  const carrierPerformance = useMemo((): CarrierPerformance[] => {
    ...
    policies.forEach(policy => {
      ...
      perf.policies++;
      const commissionAmount = policy.annualPremium * (policy.commissionPercentage / 100);
      perf.revenue += commissionAmount;
    });

    // Calculate average commission
    carrierMap.forEach((perf) => {
      perf.averageCommission = perf.policies > 0 ? perf.revenue / perf.policies : 0;
    });
  ...
  const statePerformance = useMemo((): StatePerformance[] => {
    ...
    policies.forEach(policy => {
      ...
      perf.policies++;
      perf.revenue += policy.annualPremium * (policy.commissionPercentage / 100);
    });
    ...
    stateMap.forEach((perf, state) => {
      ...
      const totalPremium = statePolicies.reduce((sum, p) => sum + p.annualPremium, 0);
      perf.averageSize = statePolicies.length > 0 ? totalPremium / statePolicies.length : 0;
    });
  ...
  const forecastMetrics = useMemo((): ForecastMetrics => {
    ...
    // Project next year
    const projectedNextYear = monthlyRunRate * 12 * Math.pow(1 + avgGrowthRate, 12);

    // Calculate pipeline value from pending policies
    const pipelineValue = policies
      .filter(p => p.status === 'pending')
      .reduce((sum, p) => sum + (p.annualPremium * p.commissionPercentage / 100), 0);
    ...
    const expectedRenewals = policies
      ...
      .reduce((sum, p) => {
        ...
        return sum + (p.annualPremium * p.commissionPercentage / 100 * 0.5);
      }, 0);

    // Growth opportunities
    const growthOpportunities = {
      highValueProducts: productPerformance
        .filter(p => p.revenue > (commissionMetrics.totalEarned / productPerformance.length) * 1.5)
        .map(p => p.product),
      underperformingCarriers: carrierPerformance
        .filter(c => c.averageCommission < (commissionMetrics.totalEarned / policies.length) * 0.5)
        .map(c => c.carrierId),
      expandableStates: [] // Would need market data
    };

    // Projected monthly commissions (next 12 months)
    const projectedMonthlyCommission: Array<{ month: string; projected: number; confidence: 'high' | 'medium' | 'low' }> = [];
    for (let i = 1; i <= 12; i++) {
    ...
    const projectedClientGrowth = Math.round(currentClients * (1 + avgGrowthRate * 12));

    // Projected policy growth
    const projectedPolicyGrowth = Math.round(policies.length * (1 + avgGrowthRate * 12));

    // Renewal forecast (next 12 months)
    const renewalForecast: Array<{ month: string; expectedRenewals: number; expectedRevenue: number }> = [];
    for (let i = 1; i <= 12; i++) {
      const futureMonth = new Date();
      futureMonth.setMonth(futureMonth.getMonth() + i);
      ...
      const policiesForRenewal = policies.filter(p => {
        if (p.status !== 'active' || !p.termLength) return false;
        const renewalDate = new Date(p.effectiveDate);
        renewalDate.setFullYear(renewalDate.getFullYear() + p.termLength);
        return renewalDate.getMonth() === futureMonth.getMonth() &&
      ...
      const expectedRenewalRevenue = policiesForRenewal.reduce((sum, p) =>
        sum + (p.annualPremium * p.commissionPercentage / 100 * 0.5), 0); // Assume 50% commission on renewals
