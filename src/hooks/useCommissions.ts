...
import { Commission, NewCommissionForm, CommissionSummary, CommissionFilters } from '../types';
...
export function useCommissions() {
  ...
  const updateCommission = (id: string, updates: Partial<Omit<Commission, 'id' | 'createdAt'>>) => {
    setCommissions((prev) =>
      prev.map((commission) =>
        commission.id === id
  ...
  const getFilteredCommissions = (filters?: CommissionFilters) => {
    ...
    return commissions.filter((commission) => {
      if (filters.startDate && new Date(commission.createdAt) < filters.startDate) return false;
      if (filters.endDate && new Date(commission.createdAt) > filters.endDate) return false;
      if (filters.carrierId && commission.carrierId !== filters.carrierId) return false;
      if (filters.product && commission.product !== filters.product) return false;
      if (filters.state && commission.client.state !== filters.state) return false;
      if (filters.minPremium && commission.annualPremium < filters.minPremium) return false;
      if (filters.maxPremium && commission.annualPremium > filters.maxPremium) return false;
      return true;
    });
  };
  ...
  const commissionSummary = useMemo((): CommissionSummary => {
    ...
    commissions.forEach((commission) => {
      ...
      const existing = carrierMap.get(commission.carrierId) || { name: carrierName, totalCommissions: 0, count: 0 };
    ...
    commissions.forEach((commission) => {
      const existing = productMap.get(commission.product) || { count: 0, totalCommissions: 0 };
    ...
    commissions.forEach((commission) => {
      const existing = stateMap.get(commission.client.state) || { count: 0, totalCommissions: 0 };
