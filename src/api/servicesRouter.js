...
export default function createServicesRouter(pool) {
  ...
  router.delete('/carriers/:id', asyncHandler(async (req, res) => {
    try {
      const { id } = req.params;
  ...
  router.post('/agents', asyncHandler(async (req, res) => {
    try {
      ...
      if (contract_comp_level < 80 || contract_comp_level > 145 || contract_comp_level % 5 !== 0) {
        return res.status(400).json({ error: 'Contract level must be between 80-145 in increments of 5' });
      }
  ...
  router.get('/comp_guide', asyncHandler(async (req, res) => {
    try {
      const result = await pool.query(`
  ...
  router.get('/comp_guide/:id', asyncHandler(async (req, res) => {
    try {
      const { id } = req.params;
      const result = await pool.query(`
        SELECT id, carrier_name, product_name, contract_level, commission_percentage, created_at, updated_at
      ...
      if (result.rows.length === 0) {
        return res.status(404).json({ error: 'Commission guide entry not found' });
      }
  ...
  router.post('/comp_guide', asyncHandler(async (req, res) => {
    try {
      const {
        carrier_name,
        product_name,
      ...
      if (!carrier_name || !product_name || !contract_level || commission_percentage === undefined) {
        return res.status(400).json({
          error: 'carrier_name, product_name, contract_level, and commission_percentage are required'
        });
      }

      if (contract_level < 80 || contract_level > 145 || contract_level % 5 !== 0) {
        return res.status(400).json({ error: 'Contract level must be between 80-145 in increments of 5' });
      }
      ...
      if (error.code === '23505') { // Unique violation
        res.status(400).json({ error: 'Commission guide entry already exists for this combination' });
      } else {
        res.status(500).json({ error: error.message });
      }
    }
  ...
  router.put('/comp_guide/:id', asyncHandler(async (req, res) => {
    try {
      const { id } = req.params;
      const {
        carrier_name,
  ...
  router.delete('/comp_guide/:id', asyncHandler(async (req, res) => {
    try {
      const { id } = req.params;
