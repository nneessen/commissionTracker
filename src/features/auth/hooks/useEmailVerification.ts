...
export const useEmailVerification = () => {
  ...
  useEffect(() => {
    ...
    if (routerEmail) {
      ...
      logger.warn(
        "EmailVerificationPending: No email found in state or sessionStorage",
        undefined,
        "Auth",
      );
    }
  ...
  const handleResend = async () => {
    ...
    if (resendCount >= MAX_RESEND_ATTEMPTS) {
      setError(
        `Maximum resend attempts (${MAX_RESEND_ATTEMPTS}) reached. Please try again later or contact support.`,
      );
      return;
    }
    ...
    try {
      ...
      logger.auth("Verification email resent", {
        email,
