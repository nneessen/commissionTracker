...
export async function getEmailOAuthTokens(): Promise<EmailOAuthToken[]> {
  ...
  const { data: tokens, error } = await supabase
    .from("user_email_oauth_tokens")
    .select(
      "id, user_id, provider, email_address, is_active, token_expiry, scopes, last_used_at, created_at, updated_at",
    )
    .eq("user_id", profile.id)
...
export async function initiateGmailOAuth(): Promise<void> {
  if (!GOOGLE_CLIENT_ID || !GOOGLE_REDIRECT_URI) {
    throw new Error(
      "Gmail OAuth not configured. Please set VITE_GOOGLE_CLIENT_ID and VITE_GOOGLE_REDIRECT_URI.",
    );
  }
