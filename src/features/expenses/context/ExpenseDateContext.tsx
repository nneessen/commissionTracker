...
const ExpenseDateContext = createContext<ExpenseDateContextType | undefined>(undefined);

export function ExpenseDateProvider({ children }: { children: React.ReactNode }) {
  ...
  const [customRange, setCustomRange] = useState<{ startDate: Date; endDate: Date } | undefined>();
...
export function useExpenseDateRange() {
  ...
  if (!context) {
    throw new Error('useExpenseDateRange must be used within ExpenseDateProvider');
  }
  return context;
}
