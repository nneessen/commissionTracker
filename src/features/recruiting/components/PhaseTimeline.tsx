...
export function PhaseTimeline({ phaseProgress, phases, onPhaseClick }: PhaseTimelineProps) {
  ...
  const getPhaseIcon = (status: string) => {
    switch (status) {
      ...
      case 'skipped':
        return <SkipForward className="h-5 w-5 text-zinc-500 dark:text-zinc-400" />;
      default:
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Overall Progress</span>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {progress?.blocked_reason && (
                    <div className="mt-2 p-3 bg-red-50 dark:bg-red-950/30 rounded-lg border border-red-200 dark:border-red-800">
                      ...
                      <span className="text-sm text-red-700 dark:text-red-400">{progress.blocked_reason}</span>
                    </div>
                  )}

                  {progress?.notes && !progress.blocked_reason && (
                    <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400 italic">{progress.notes}</p>
                  )}
