...
import { Building2, Users, Trophy, TrendingUp, BarChart3, AlertCircle } from "lucide-react";
...
export const OrgMetricsSection: React.FC<OrgMetricsSectionProps> = ({
  ...
  // IMO admin view: Show IMO metrics + Production breakdown
  if (isImoAdmin) {
    return (
      <div className="grid gap-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-[300px_1fr]">
        <ImoMetricsPanel />
  ...
  // Agency owner view: Show Agency metrics only
  if (isAgencyOwner) {
    return (
      <div className="grid gap-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-[300px_1fr]">
        <AgencyMetricsPanel />
        <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-3 flex items-center justify-center">
          <div className="text-center text-zinc-400">
            <TrendingUp className="h-8 w-8 mx-auto mb-2 opacity-50" />
            <div className="text-[11px]">Production charts coming soon</div>
          </div>
        </div>
      </div>
    );
