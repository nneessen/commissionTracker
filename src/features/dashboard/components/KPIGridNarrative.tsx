 ...
 */
function generateInsight(label: string, value: string | number, category: string): string {
...
function getTrend(label: string): 'up' | 'down' | 'neutral' {
  ...
  if (label.includes('Profit') || label.includes('Commission') || label.includes('Policies')) {
 ...
 */
function getProgress(label: string, value: string | number): number | undefined {
...
function getAccentColor(category: string): string {
  ...
  if (lowerCategory.includes('financial')) return 'border-violet-500 dark:border-violet-400';
  if (lowerCategory.includes('production')) return 'border-blue-500 dark:border-blue-400';
  if (lowerCategory.includes('commission') || lowerCategory.includes('metric'))
    return 'border-emerald-500 dark:border-emerald-400';
  if (lowerCategory.includes('client')) return 'border-cyan-500 dark:border-cyan-400';
 ...
 */
export const KPIGridNarrative: React.FC<DetailedKPIGridProps> = ({ sections }) => {
        ...
        {heroKPI && (
          <div className="pb-4 mb-4 border-b border-border/40">
            <NarrativeInsight
              label={heroKPI.label}
              value={heroKPI.value}
              insight={generateInsight(heroKPI.label, heroKPI.value, heroKPI.category)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sections.map((section, sectionIndex) => (
            <div key={sectionIndex} className="space-y-3">
              {/* Category Header */}
              <h3 className="text-xs uppercase tracking-wider font-bold text-muted-foreground/80 mb-2">
                {section.category}
              </h3>
              ...
              {section.kpis.map((kpi, kpiIndex) => (
                <NarrativeInsight
                  ...
                  label={kpi.label}
                  value={kpi.value}
                  insight={generateInsight(kpi.label, kpi.value, section.category)}
        ...
        <div className="mt-6 pt-4 border-t border-border/40">
          <div className="flex items-start gap-3 p-3 rounded-lg bg-primary/5">
            <div className="text-xs leading-relaxed text-muted-foreground">
              <span className="font-semibold text-foreground">Key Takeaway:</span> Your metrics show
              overall positive momentum. Continue monitoring pace targets and expense efficiency to
              optimize performance this period.
            </div>
          </div>
        </div>
      </CardContent>
