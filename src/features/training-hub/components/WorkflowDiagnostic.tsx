  ...
  return (
    <div className="w-full rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
      <div className="px-3 py-2 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 rounded-t-lg">
        <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Workflow System Diagnostic</h3>
      </div>
      <div className="p-3 space-y-3">
        <div className="flex gap-1.5">
          <Button onClick={runDiagnostic} disabled={loading} size="sm" className="h-6 text-[10px]">
            {loading ? 'Running...' : 'Run Diagnostic'}
          </Button>
          <Button onClick={clearCache} variant="destructive" size="sm" className="h-6 text-[10px]">
            Clear Cache & Reload
          </Button>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="p-2 rounded-md border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/30">
              <div className="flex items-start gap-2">
                ...
                <div className="text-[11px]">
                  <span className="font-medium text-zinc-700 dark:text-zinc-300">Authentication:</span>{' '}
                  {results.auth.isAuthenticated ? (
                    <span className="text-emerald-600 dark:text-emerald-400">✅ Logged in as {results.auth.email}</span>
                  ) : (
            ...
            <div className="p-2 rounded-md border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/30">
              <div className="flex items-start gap-2">
                <AlertCircle className="h-3.5 w-3.5 mt-0.5 text-zinc-500" />
                <div className="text-[11px]">
                  <span className="font-medium text-zinc-700 dark:text-zinc-300">Database Workflows:</span>{' '}
                  <span className="text-zinc-600 dark:text-zinc-400">{results.database.workflowCount} workflows found</span>
            ...
            {/* Recommendations */}
            <div>
              <h4 className="text-[11px] font-semibold text-zinc-700 dark:text-zinc-300 mb-1">Recommendations:</h4>
              <div className="space-y-0.5">
                {results.recommendations.map((rec: string, idx: number) => (
                  <div key={idx} className="text-[11px] text-zinc-600 dark:text-zinc-400">
                    {rec}
                  </div>
                ))}
              </div>
            ...
            <details className="text-[10px]">
              <summary className="cursor-pointer font-medium text-zinc-600 dark:text-zinc-400">Raw Diagnostic Data</summary>
