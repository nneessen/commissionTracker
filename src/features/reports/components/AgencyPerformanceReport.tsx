...
import { Loader2, TrendingUp, TrendingDown, Users, FileText, DollarSign } from 'lucide-react';
...
import { ReportErrorBoundary, QueryErrorAlert, ReportQueryError } from './ReportErrorBoundary';
...
function AgencyPerformanceReportContent({ agencyId, dateRange }: AgencyPerformanceReportProps) {
  ...
  const agentComparisonData = useMemo(() => {
    ...
    return agentProduction.slice(0, 10).map((agent) => ({
      label: agent.agent_name.length > 12
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-2 text-sm text-zinc-500">Loading agency performance data...</span>
      </div>
    );
  }
  ...
  if (hasCriticalError) {
    return (
      <ReportQueryError
        message={errorPerformance instanceof Error ? errorPerformance.message : 'Failed to load performance data'}
        onRetry={handleRetry}
      />
    );
  }
  ...
  if (!performanceReport) {
    return (
      <div className="p-4 text-center text-sm text-zinc-500">
        No performance data available. You may not have permission to view this report.
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <Card className="bg-white dark:bg-zinc-900">
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] text-zinc-500 uppercase">New Premium</p>
                <p className="text-sm font-semibold">{formatCurrency(summary.total_new_premium)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white dark:bg-zinc-900">
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] text-zinc-500 uppercase">Commissions</p>
                <p className="text-sm font-semibold">{formatCurrency(summary.total_commissions)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white dark:bg-zinc-900">
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] text-zinc-500 uppercase">New Policies</p>
                <p className="text-sm font-semibold">{summary.total_new_policies.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white dark:bg-zinc-900">
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] text-zinc-500 uppercase">New Agents</p>
                <p className="text-sm font-semibold">{summary.total_new_agents.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm font-semibold">{summary.total_lapsed.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white dark:bg-zinc-900">
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              <div className={`p-1.5 rounded ${netGrowthPositive ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-red-100 dark:bg-red-900/30'}`}>
                {netGrowthPositive ? (
              ...
              <div>
                <p className="text-[10px] text-zinc-500 uppercase">Net Growth</p>
                <p className={`text-sm font-semibold ${netGrowthPositive ? 'text-emerald-600' : 'text-red-600'}`}>
                  {netGrowthPositive ? '+' : ''}{formatCurrency(summary.net_growth)}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader className="py-3 px-4">
          <CardTitle className="text-sm font-semibold">Monthly Production Trend</CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
          <TrendLineChart
            data={trendData}
            lines={[
              { dataKey: 'premium', name: 'New Premium', color: '#3b82f6', format: 'currency' },
              { dataKey: 'commissions', name: 'Commissions', color: '#10b981', format: 'currency' },
            ]}
            height={250}
          />
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-zinc-500 mb-2">Top Agents by Premium</p>
                <BarComparisonChart
                  data={agentComparisonData}
                  bars={[
                    { dataKey: 'premium', name: 'Premium', color: '#3b82f6', format: 'currency' },
                  ]}
                  height={200}
                  layout="vertical"
                  showLegend={false}
                      ...
                      <TableRow key={agent.agent_id} className="text-xs">
                        <TableCell className="py-1.5">
                          <div>
                            ...
                            <p className="text-[10px] text-zinc-500">{agent.agent_email}</p>
                          </div>
                        </TableCell>
                        <TableCell className="py-1.5 text-right">{agent.active_policies}</TableCell>
                        <TableCell className="py-1.5 text-right">{formatCurrency(agent.total_annual_premium)}</TableCell>
                        <TableCell className="py-1.5 text-right">
                          <span className="text-zinc-500">{agent.pct_of_agency_production}%</span>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <Table>
              <TableHeader>
                <TableRow className="text-xs">
                  ...
                  <TableHead className="py-2 text-right">New Policies</TableHead>
              ...
              <TableBody>
                {performanceReport.months.map((month) => (
                  <TableRow key={month.month_start} className="text-xs">
                    <TableCell className="py-1.5 font-medium">{month.month_label}</TableCell>
                    <TableCell className="py-1.5 text-right">{month.new_policies}</TableCell>
                    <TableCell className="py-1.5 text-right">{formatCurrency(month.new_premium)}</TableCell>
                    <TableCell className="py-1.5 text-right">{formatCurrency(month.commissions_earned)}</TableCell>
                    <TableCell className="py-1.5 text-right text-red-600">{month.policies_lapsed}</TableCell>
                    <TableCell className="py-1.5 text-right">
                      <span className={month.net_premium_change >= 0 ? 'text-green-600' : 'text-red-600'}>
                        {month.net_premium_change >= 0 ? '+' : ''}{formatCurrency(month.net_premium_change)}
                      </span>
                    </TableCell>
                  </TableRow>
                ))}
