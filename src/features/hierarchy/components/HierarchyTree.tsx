 ...
 */
function TreeNode({ node, level, onNodeClick, searchTerm, expandedNodes, toggleNode }: TreeNodeProps) {
    ...
    <div className="w-full">
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex items-center gap-3">
          ...
          <Badge variant="outline" className="text-[10px] px-1 py-0">L{node.hierarchy_depth}</Badge>
          {hasChildren && (
            <span className="text-[10px] text-muted-foreground">
              {node.direct_downline_count} direct
            </span>
          )}
          {node.override_earnings !== undefined && node.override_earnings > 0 && (
...
export function HierarchyTree({ nodes, onNodeClick, className }: HierarchyTreeProps) {
  ...
  const [selectedAgent, setSelectedAgent] = useState<HierarchyNode | null>(null);
