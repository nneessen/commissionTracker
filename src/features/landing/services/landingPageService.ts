...
export async function getLandingPageSettings(
  ...
  try {
    ...
    logger.error('Exception fetching landing page settings', err instanceof Error ? err : String(err));
    throw err;
  }
}
...
export async function upsertLandingPageSettings(
  ...
  try {
    ...
    logger.error('Exception upserting landing page settings', err instanceof Error ? err : String(err));
    throw err;
  }
}
...
export async function deleteLandingPageSettings(imoId: string): Promise<void> {
  try {
    const { error } = await supabase.from(TABLE_NAME).delete().eq('imo_id', imoId);
    ...
    logger.error('Exception deleting landing page settings', err instanceof Error ? err : String(err));
    throw err;
  }
}
...
export async function uploadLandingPageAsset(
  ...
  try {
    ...
    logger.error('Exception uploading landing page asset', err instanceof Error ? err : String(err));
    throw err;
  }
}
...
export async function deleteLandingPageAsset(url: string): Promise<void> {
  try {
    ...
    logger.error('Exception deleting landing page asset', err instanceof Error ? err : String(err));
    throw err;
  }
}
...
export async function deleteAllLandingPageAssets(imoId: string): Promise<void> {
  try {
    ...
    if (files && files.length > 0) {
      const filePaths = files.map((f: { name: string }) => `${imoId}/${f.name}`);
    ...
    logger.error('Exception deleting all landing page assets', err instanceof Error ? err : String(err));
    throw err;
  }
}
