...
class CommissionService {
  ...
  async calculateCommissionWithCompGuide(data: {
    ...
    if (!contractCompLevel && userId) {
      try {
        ...
        logger.warn('Could not get user contract comp level', error instanceof Error ? error : String(error), 'CommissionService');
      }
    }
  ...
  async getCommissionMetrics(userId?: string, startDate?: Date, endDate?: Date): Promise<{
  ...
  async getCommissionsWithChargebackRisk(userId?: string): Promise<CommissionWithChargebackRisk[]> {
    ...
    const commissions = userId ? await this.getCommissionsByUser(userId) : await this.getAll();
  ...
  private transformToDB(data: Partial<CreateCommissionData>, isUpdate = false): any {
    ...
    // Map userId to user_id for database
    const userId = data.userId;
    if (userId !== undefined) {
