#!/bin/bash
# File: scripts/generate-continuation-prompt.sh
# Generates comprehensive continuation prompt for new Claude Code conversation

set -e

TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
PROMPT_DIR="plans/continuation-prompts"
PROMPT_FILE="${PROMPT_DIR}/continue-${TIMESTAMP}.md"
USER_NOTE="$1"
...
if [ -d "plans/active" ]; then
    find plans/active -name "*.md" -type f 2>/dev/null | while read -r plan; do
...
find plans/ -maxdepth 1 -name "*.md" -type f ! -name "*completed*" ! -name "*COMPLETED*" 2>/dev/null | while read -r plan; do
    echo "- **$(basename "$plan")**" >> "$PROMPT_FILE"
    printf '  ```\n' >> "$PROMPT_FILE"
    head -20 "$plan" >> "$PROMPT_FILE"
    printf '  ```\n\n' >> "$PROMPT_FILE"
...
find plans/ -name "*.md" -type f -mtime -7 ! -path "*/completed/*" ! -path "*/COMPLETED/*" 2>/dev/null | while read -r plan; do
...
if [ -d ".serena/memories" ]; then
    ls -1 .serena/memories/*.md 2>/dev/null | while read -r mem; do
