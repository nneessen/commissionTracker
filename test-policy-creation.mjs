...
async function testPolicyCreation() {
  ...
  const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
    email: 'nneessen@gmail.com', // Using the email from your env
    password: 'password123' // You'll need to update this
  });

  if (authError) {
    console.error('Authentication failed:', authError);
    console.log('\nPlease update the password in the script with your valid credentials.');
    console.log('Or create a test user with: supabase.auth.signUp({ email: "test@example.com", password: "test123" })');
    return;
  }
