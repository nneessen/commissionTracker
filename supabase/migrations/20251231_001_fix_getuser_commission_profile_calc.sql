-- Migration: 20251231_001_fix_getuser_commission_profile_calc.sql
-- Purpose: Fix getuser_commission_profile to calculate from actual policies using comp_guide table
-- Replaces: 20251213_003_simplified_getuser_commission_profile.sql (which had hardcoded defaults)
--
-- CRITICAL FIX: This migration corrects the table name from commission_rates to comp_guide
-- and restores the premium-weighted commission rate calculation logic.

-- Drop existing simplified function
DROP FUNCTION IF EXISTS public.getuser_commission_profile(uuid, integer);

-- Create corrected function with proper calculation logic
...
BEGIN
    ...
    -- Get user's current contract level using the correct column name
...
Uses comp_guide table for commission rates. Data quality levels:
