...
-- Migration: Allow error -> verified transition in custom domain state machine
--
-- Problem: When a domain is in "error" state (e.g., DNS verification failed),
-- and the user fixes their DNS records and retries verification, the verify
-- function successfully validates DNS but cannot transition to "verified"
-- because the state machine only allowed error -> pending_dns.
--
-- Fix: Allow error -> verified as a valid transition for successful retry.
...
BEGIN
  ...
  -- error -> pending_dns | verified (retry verification can succeed directly)
  IF NOT (
    ...
    (v_old_status = 'error' AND p_new_status IN ('pending_dns', 'verified')) -- Allow retry to pending_dns OR verified
  ) THEN
...
COMMENT ON FUNCTION admin_update_domain_status IS
  'Transitions domain status with state machine enforcement. Allows error -> verified for successful retry verification. Clears last_error on successful transitions. Called by Edge Functions with service_role only.';
