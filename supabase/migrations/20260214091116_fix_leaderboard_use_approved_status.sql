...
-- Fix: IP should use 'approved' status, not 'active' status
--
-- Issue: Database uses 'approved' status for issued policies, not 'active'
--
-- Business Logic:
...
-- Comments
COMMENT ON FUNCTION get_leaderboard_data IS 'Individual agent leaderboard. IP = approved policies by effective_date in range. AP = ALL policies (any status) by submit_date in range.';
COMMENT ON FUNCTION get_team_leaderboard_data IS 'Team leaderboard. IP = approved policies by effective_date in range. AP = ALL policies (any status) by submit_date in range.';
COMMENT ON FUNCTION get_agency_leaderboard_data IS 'Agency leaderboard. IP = approved policies by effective_date in range. AP = ALL policies (any status) by submit_date in range.';
