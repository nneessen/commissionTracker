...
-- Fix: Correct column name from 'submitted_date' (doesn't exist) to 'submit_date' in leaderboard RPCs
--
-- Bug: The policies table has 'submit_date', not 'submitted_date'.
-- The carrier_contracts table has 'submitted_date', which caused confusion.
-- This migration fixes get_leaderboard_data, get_agency_leaderboard_data, and get_team_leaderboard_data.

-- ============================================================================
-- 1. Fix individual agent leaderboard
-- ============================================================================
...
BEGIN
  ...
  -- FIX: Changed from p.submitted_date to p.submit_date (correct column name)
  ap_data AS (
    SELECT
      p.user_id,
...
BEGIN
  ...
  -- Calculate combined AP per agency
  -- FIX: Changed from p.submitted_date to p.submit_date (correct column name)
  ap_data AS (
    SELECT
      aa.agency_id,
...
BEGIN
  ...
  -- Calculate combined AP per team
  -- FIX: Changed from p.submitted_date to p.submit_date (correct column name)
  ap_data AS (
    SELECT
      tm.leader_id,
...
-- Comments
-- ============================================================================
COMMENT ON FUNCTION get_leaderboard_data IS 'Returns individual agent leaderboard with IP (Issued Premium - active policies with paid commissions) and AP (Annual Premium - pending policies). Fixed: uses correct submit_date column.';
COMMENT ON FUNCTION get_agency_leaderboard_data IS 'Returns agency leaderboard ranking agencies as units by combined IP/AP of all their agents. Fixed: uses correct submit_date column.';
COMMENT ON FUNCTION get_team_leaderboard_data IS 'Returns team leaderboard ranking teams (leader + direct downlines) as units by combined IP/AP. Fixed: uses correct submit_date column.';
