...
-- Expand admin_deleteuser permission check to allow trainers/contracting_managers
-- to delete users with the 'recruit' role.
...
BEGIN
  ...
  v_is_staff := v_caller_roles && ARRAY['trainer', 'contracting_manager'];

  IF NOT COALESCE(v_is_admin, false) AND NOT COALESCE(v_is_staff, false) THEN
    RAISE EXCEPTION 'Only admins and staff can delete users';
  END IF;

  -- If staff (not admin), verify target is a recruit
  IF NOT COALESCE(v_is_admin, false) AND COALESCE(v_is_staff, false) THEN
    SELECT roles INTO v_target_roles FROM user_profiles WHERE id = target_user_id;
