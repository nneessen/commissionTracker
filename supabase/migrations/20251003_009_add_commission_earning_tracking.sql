-- supabase/migrations/20251003_009_add_commission_earning_tracking_FIXED.sql
...
-- First verify commission_amount column exists before using it
DO $$
DECLARE
  v_commission_amount_exists BOOLEAN;
BEGIN
  ...
  ) INTO v_commission_amount_exists;

  IF v_commission_amount_exists THEN
    -- For existing commissions, set unearned_amount = commission_amount
    -- (assumes they were just created and nothing earned yet)
    UPDATE public.commissions
    SET unearned_amount = commission_amount
    WHERE unearned_amount IS NULL;

    RAISE NOTICE 'Initialized unearned_amount for existing records';
  ELSE
    RAISE WARNING 'commission_amount column does not exist - cannot initialize unearned_amount';
  END IF;
END $$;
...
'Portion of advance commission that has been earned through client payments. Formula: (commission_amount / advance_months) × months_paid';

COMMENT ON COLUMN public.commissions.unearned_amount IS
'Portion of advance commission that is still unearned. Formula: commission_amount - earned_amount. This represents chargeback exposure.';
...
BEGIN
  -- Validate inputs
  IF p_commission_amount < 0 OR p_advance_months <= 0 OR p_months_paid < 0 THEN
    RETURN 0;
  END IF;
  ...
  v_monthly_earning_rate := p_commission_amount / p_advance_months;
...
'Calculate earned commission based on months paid. Formula: (commission_amount / advance_months) × months_paid';
...
DECLARE
  v_earned_amount DECIMAL;
BEGIN
  v_earned_amount := public.calculate_earned_amount(p_commission_amount, p_advance_months, p_months_paid);
  RETURN p_commission_amount - v_earned_amount;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

COMMENT ON FUNCTION public.calculate_unearned_amount IS
'Calculate unearned commission (chargeback exposure). Formula: commission_amount - earned_amount';
...
CREATE TRIGGER trigger_update_commission_earned
  BEFORE INSERT OR UPDATE OF months_paid, commission_amount, advance_months
...
'Automatically recalculates earned_amount and unearned_amount when months_paid or commission_amount changes';
...
SELECT
  ...
  c.policy_id,
  c.user_id,
  c.commission_amount as advance_amount,
  c.advance_months,
  c.months_paid,
  c.earned_amount,
  c.unearned_amount,
  c.last_payment_date,
  ROUND((c.months_paid::DECIMAL / NULLIF(c.advance_months, 0)) * 100, 2) as percentage_earned,
  ...
  CASE
    WHEN c.months_paid = 0 THEN 'HIGH'
...
SELECT
  ...
  SUM(c.commission_amount) as total_advances,
...
BEGIN
  -- Check if commission_amount exists for summary
  SELECT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'public'
