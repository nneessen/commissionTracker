-- /home/nneessen/projects/commissionTracker/supabase/migrations/20251215_003_fix_admin_deleteuser_final.sql
-- Migration: Fix admin_deleteuser function (final)
-- Date: 2024-12-15
--
-- Previous migrations had issues:
-- - 20241213_005: References non-existent table 'recruiting_checklists'
-- - 20251213_005: References user_profiles.user_id which was removed by unify_user_ids
--
-- This version:
-- - Uses correct table: recruit_checklist_progress
-- - Uses target_user_id directly for auth.users (since user_profiles.id = auth.users.id after unification)
-- - No reference to user_profiles.user_id (column removed)

-- Drop existing function to ensure clean replacement
DROP FUNCTION IF EXISTS admin_deleteuser(uuid);
...
BEGIN
  ...
  -- Recruiting progress (CORRECT table name: recruit_checklist_progress, NOT recruiting_checklists)
  ...
  -- Commissions - CRITICAL: use correct column names
  ...
  -- Handle user_profiles self-references (recruiter_id, upline_id, archived_by)
  -- Set these to NULL for other users who reference the deleted user
  ...
  -- After unify_user_ids migration: user_profiles.id = auth.users.id
  -- So we use target_user_id directly
...
GRANT EXECUTE ON FUNCTION admin_deleteuser(uuid) TO authenticated;

-- Add comment
COMMENT ON FUNCTION admin_deleteuser IS 'Deletes a user and all related data. Only callable by admins. Uses verified column names from database schema. After unify_user_ids: user_profiles.id = auth.users.id';
