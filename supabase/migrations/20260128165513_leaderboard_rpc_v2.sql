...
-- Leaderboard RPC v2: Add AP metric, agency rankings, and team rankings
--
-- Changes:
-- 1. Update get_leaderboard_data to include AP (Annual Premium - pending policies)
-- 2. Add get_agency_leaderboard_data - ranks agencies as units
-- 3. Add get_team_leaderboard_data - ranks teams (leader + downlines) as units
...
-- ============================================================================
-- 1. Update individual agent leaderboard to include AP
-- ============================================================================
...
BEGIN
  ...
  ap_data AS (
    ...
    WHERE p.status = 'pending'
      ...
      AND p.submitted_date::date >= p_start_date
      AND p.submitted_date::date <= p_end_date
    GROUP BY p.user_id
  ),
...
-- ============================================================================
-- 2. Agency Leaderboard - Ranks agencies as units
-- ============================================================================
...
-- ============================================================================
-- 3. Team Leaderboard - Ranks teams (leader + downlines) as units
-- ============================================================================
...
-- Update comments
-- ============================================================================
COMMENT ON FUNCTION get_leaderboard_data IS 'Returns individual agent leaderboard with IP (Issued Premium - active policies with paid commissions) and AP (Annual Premium - pending policies).';
COMMENT ON FUNCTION get_agency_leaderboard_data IS 'Returns agency leaderboard ranking agencies as units by combined IP/AP of all their agents.';
COMMENT ON FUNCTION get_team_leaderboard_data IS 'Returns team leaderboard ranking teams (leader + direct downlines) as units by combined IP/AP.';
