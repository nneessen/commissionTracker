...
-- Fix: Remove overly strict source='imported' filter from existence check
-- This was causing all products to be skipped even when no age rule sets existed
...
BEGIN
  ...
  LOOP
    ...
    -- FIX: Removed source = 'imported' - we want to skip if ANY age rule set exists
    ...
    IF v_product.min_age IS NOT NULL THEN
      ...
      INSERT INTO underwriting_rules (
        ...
        'ineligible',
        'decline',
        'Client age is below minimum age of ' || v_product.min_age || ' for this product',
...
'Generate draft age eligibility rule sets from product min_age/max_age. Fixed: removed overly strict source filter from existence check.';
