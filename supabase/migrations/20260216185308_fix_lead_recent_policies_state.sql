-- Drop and recreate get_lead_recent_policies with client_state column
-- Previous migration added the state column to clients but couldn't alter
-- the function return type in-place.
DROP FUNCTION IF EXISTS get_lead_recent_policies(uuid, integer);

CREATE FUNCTION get_lead_recent_policies(
