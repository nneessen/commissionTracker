-- supabase/migrations/20251227_001_fix_handle_new_user_final.sql
--
-- FIX: User creation fails with "Database error creating new user"
-- Root cause: The handle_new_user() trigger crashes without proper error handling
--
-- This migration creates an ultra-robust version that:
-- 1. Has exception handling around ALL operations
-- 2. Gracefully handles JSON parsing failures
-- 3. Uses ON CONFLICT for idempotent inserts
-- 4. Logs errors for debugging
-- 5. Only inserts columns guaranteed to exist with safe defaults

-- ============================================================================
-- Drop and recreate the trigger function with ultra-robust error handling
-- ============================================================================
...
Ultra-robust version with comprehensive exception handling.
Uses first_name/last_name columns, respects roles from user_metadata.
Updated 2024-12-27 to fix user creation failures.';
