...
-- Fix: IP (Issued Premium) should filter by policy effective_date, not commission payment_date
--
-- Bug: IP calculation was using c.payment_date (when commission was paid) to filter.
-- This caused policies with 2025 effective dates but 2026 payment dates to appear.
--
-- Correct logic:
-- - IP = Active policies with paid commissions, filtered by POLICY effective_date
-- - AP = Pending policies, filtered by POLICY submit_date
--
-- Example of bug: James Wadleigh had 4 policies with effective_date in 2025
-- but payment_date in 2026-02-02. He showed 6932 IP when he should show 0.

-- ============================================================================
-- 1. Fix individual agent leaderboard - IP uses effective_date
-- ============================================================================
...
BEGIN
  ...
  -- Calculate IP (Issued Premium) - active policies with paid advance commissions
  -- FIX: Filter by policy effective_date, NOT commission payment_date
  ip_data AS (
    SELECT
      p.user_id,
  ...
  -- Calculate AP (Annual Premium) - pending policies by submit_date
  ap_data AS (
    SELECT
      p.user_id,
...
-- ============================================================================
-- 2. Fix agency leaderboard - IP uses effective_date
-- ============================================================================
...
BEGIN
  ...
  -- Calculate combined AP per agency by submit_date
  ap_data AS (
    SELECT
      aa.agency_id,
      SUM(COALESCE(p.annual_premium, 0)) AS total_ap,
  ...
  -- Count prospects per agency (based on recruiter)
  prospect_data AS (
    SELECT
      aa.agency_id,
...
-- ============================================================================
-- 3. Fix team leaderboard - IP uses effective_date
-- ============================================================================
...
BEGIN
  ...
  team_members AS (
    SELECT
      tl.id AS leader_id,
      tl.id AS member_id  -- Include leader themselves
    FROM team_leaders tl
    UNION ALL
    SELECT
      tl.id AS leader_id,
      d.id AS member_id  -- Include direct downlines
    FROM team_leaders tl
  ...
  -- Count team size (leader + downlines)
  team_sizes AS (
    SELECT
      tm.leader_id,
  ...
  -- Calculate combined AP per team by submit_date
  ap_data AS (
    SELECT
      tm.leader_id,
      SUM(COALESCE(p.annual_premium, 0)) AS total_ap,
  ...
  -- Count prospects per team (based on recruiter being a team member)
  prospect_data AS (
    SELECT
      tm.leader_id,
      COUNT(DISTINCT r.id) AS prospect_count
...
-- Comments
-- ============================================================================
COMMENT ON FUNCTION get_leaderboard_data IS 'Agent leaderboard. IP = active policies by effective_date. AP = pending policies by submit_date. Fixed: uses effective_date for IP (not payment_date).';
COMMENT ON FUNCTION get_agency_leaderboard_data IS 'Agency leaderboard. IP = active policies by effective_date. AP = pending policies by submit_date. Fixed: uses effective_date for IP (not payment_date).';
COMMENT ON FUNCTION get_team_leaderboard_data IS 'Team leaderboard. IP = active policies by effective_date. AP = pending policies by submit_date. Fixed: uses effective_date for IP (not payment_date).';
