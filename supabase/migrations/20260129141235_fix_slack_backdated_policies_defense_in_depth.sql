-- supabase/migrations/20260129141235_fix_slack_backdated_policies_defense_in_depth.sql
-- Defense in depth: Ensure trigger passes submit_date to edge function
-- AND edge function validates it as a backup

-- ============================================================================
-- Recreate trigger function with submit_date in payload
-- ============================================================================
...
BEGIN
  ...
  -- Check submit_date: only post for today's submissions (America/New_York timezone)
  -- This is the PRIMARY check - edge function has a backup check
  ...
  -- NOTE: submitDate is now included for defense-in-depth validation in edge function
...
DEFENSE IN DEPTH (2026-01-29):
1. PRIMARY CHECK: Trigger validates submit_date = today (America/New_York timezone)
2. BACKUP CHECK: Edge function also validates submit_date before posting
