...
-- Fix: admin_deleteuser references non-existent 'user_id' column
-- Fix: admin_deleteuser references non-existent 'expense_categories' table
-- In this schema, user_profiles.id IS the auth.users.id (no separate user_id column)
--
-- Also drops duplicate admin_delete_user function (with underscore) - the code uses
-- admin_deleteuser (no underscore) via the RPC call.

-- Drop the duplicate function if it exists
DROP FUNCTION IF EXISTS admin_delete_user(uuid);

-- Fix the admin_deleteuser function
