-- supabase/migrations/20260129093145_registration_with_password.sql
-- Update submit_recruit_registration to accept auth_user_id
-- When auth_user_id is provided, UPDATE the existing profile (created by auth trigger)
-- instead of creating a new one. This allows recruits to create their own account
-- with a password during registration.
...
COMMENT ON FUNCTION submit_recruit_registration(UUID, JSONB, UUID) IS
  'Submits recruit registration form. If p_auth_user_id is provided, updates that existing profile.
   Otherwise creates new profile or updates existing recruit. Handles empty strings gracefully.';
