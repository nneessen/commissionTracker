-- Migration: 20260110_011_fix_commission_rate_from_policies.sql
-- Purpose: Fix getuser_commission_profile to use policies.commission_percentage as primary source
--
-- ROOT CAUSE: The function was ignoring policies.commission_percentage and only looking at
-- comp_guide/products tables. When policies lacked product_id or comp_guide lacked entries,
-- it fell back to DEFAULT rates instead of using the actual commission rate stored on policies.
--
-- FIX: Changed cascade order to:
--   1. policies.commission_percentage (actual rate on the policy - PRIMARY SOURCE)
--   2. comp_guide.commission_percentage (lookup by product/carrier/contract)
--   3. products.commission_percentage (product default)

-- Drop existing function
DROP FUNCTION IF EXISTS public.getuser_commission_profile(uuid, integer);

-- Create corrected function that uses policies.commission_percentage as primary source
...
FIX (20260110): Now uses policies.commission_percentage as PRIMARY source, falling back to comp_guide/products.
This ensures policies with commission rates stored directly on them are counted, even without product_id.
