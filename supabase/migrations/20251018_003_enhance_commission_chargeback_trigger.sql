...
BEGIN
  ...
  v_monthly_earning_rate := CASE
    WHEN v_commission.advance_months > 0 THEN
      v_commission.amount / v_commission.advance_months
    ELSE 0
  END;
  ...
  v_chargeback_amount := v_commission.amount - v_earned_amount;
  ...
  RETURN jsonb_build_object(
    ...
    'months_paid', v_months_paid,
    'advance_amount', v_commission.amount,
    'earned_amount', v_earned_amount,
    'chargeback_amount', v_chargeback_amount,
    'chargeback_reason', v_chargeback_reason
  );
...
SELECT
  ...
  ROUND(
    (SUM(c.chargeback_amount) / NULLIF(SUM(c.amount), 0)) * 100,
    2
