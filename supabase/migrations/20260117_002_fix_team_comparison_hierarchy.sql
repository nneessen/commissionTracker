...
-- Fix: get_team_comparison_report should use hierarchy-based team calculation
--
-- Problem: This function was using flat agency_id lookup which misses nested downlines
-- in MLM structures. The dashboard's get_agency_dashboard_metrics was fixed on 2026-01-15
-- to use hierarchy_path traversal, but this function was not updated.
--
-- Solution: Use hierarchy_path to find all users in each agency owner's downline tree,
-- matching the approach used in get_agency_dashboard_metrics.

-- =====================================================
-- Drop and recreate get_team_comparison_report with hierarchy support
-- =====================================================
...
BEGIN
  ...
  -- Note: This includes ALL users in the IMO, not just those with agency_id set
  ...
  WITH agency_owners AS (
    -- Get each agency and its owner's hierarchy_path for team lookup
    SELECT
      a.id AS agency_id,
      ...
      COALESCE(owner.hierarchy_path, owner.id::text) AS owner_hierarchy_path
    FROM agencies a
  ...
  agency_metrics AS (
    SELECT
      ...
      ao.agency_code,
      ao.owner_name,
      -- Team members: owner + all users in owner's hierarchy (MLM downlines)
      -- This matches the approach in get_agency_dashboard_metrics
      COALESCE(team_stats.agent_count, 0)::bigint AS metric_agent_count,
      ...
      CASE
        WHEN COALESCE(team_stats.agent_count, 0) > 0
        THEN ROUND(COALESCE(policy_stats.premium_total, 0) / team_stats.agent_count, 2)
        ELSE 0
    ...
    -- LATERAL subquery for team member count using hierarchy_path
    LEFT JOIN LATERAL (
      ...
      WHERE up.approval_status = 'approved'
        ...
        AND (
          up.id = ao.owner_id  -- The owner
          OR up.hierarchy_path LIKE ao.owner_hierarchy_path || '.%'  -- All descendants
        )
    ) team_stats ON true
    -- LATERAL subquery for policy metrics using hierarchy_path
    LEFT JOIN LATERAL (
      SELECT
    ...
    -- LATERAL subquery for lapse metrics using hierarchy_path
    LEFT JOIN LATERAL (
    ...
    -- LATERAL subquery for commission metrics using hierarchy_path
    LEFT JOIN LATERAL (
...
UPDATED 2026-01-17: Now uses hierarchy_path to recursively include all downlines
in each agency owner''s team, matching the approach in get_agency_dashboard_metrics.
This properly supports MLM team structures where downlines can have their own downlines.';
