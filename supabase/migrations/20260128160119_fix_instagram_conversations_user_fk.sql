...
-- Fix: instagram_conversations.priority_set_by FK blocks user deletion
-- The FK constraint lacks ON DELETE SET NULL, causing admin_deleteuser to fail

-- Step 1: Fix the FK constraint to allow cascading SET NULL on user deletion
ALTER TABLE instagram_conversations
DROP CONSTRAINT IF EXISTS instagram_conversations_priority_set_by_fkey;

ALTER TABLE instagram_conversations
ADD CONSTRAINT instagram_conversations_priority_set_by_fkey
FOREIGN KEY (priority_set_by) REFERENCES auth.users(id) ON DELETE SET NULL;

-- Step 2: Update admin_deleteuser function to also clear instagram_conversations
-- (belt and suspenders approach - both constraint and explicit clear)
...
BEGIN
  ...
  -- Instagram conversations (clear priority_set_by reference)
  UPDATE instagram_conversations
  SET priority_set_by = NULL
  WHERE priority_set_by = target_user_id;
...
'Deletes a user and all related data. Updated 2026-01-28 to handle instagram_conversations.priority_set_by.';
