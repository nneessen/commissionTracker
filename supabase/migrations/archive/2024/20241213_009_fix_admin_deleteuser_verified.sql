-- Migration: Fix admin_deleteuser function with verified schema
-- Date: 2024-12-13
-- Description: Create admin delete user function using verified column names from database.types.ts
--
-- This migration fixes all previous column name errors by using the actual schema:
-- - commissions.user_id (NOT agent_id)
-- - override_commissions.base_agent_id and override_agent_id (NOT agent_id)
-- - recruit_checklist_progress (NOT recruiting_checklists)
-- - All verified from src/types/database.types.ts
...
-- Add comment
COMMENT ON FUNCTION admin_deleteuser IS 'Deletes a user and all related data. Only callable by admins. Uses verified column names from database schema.';
