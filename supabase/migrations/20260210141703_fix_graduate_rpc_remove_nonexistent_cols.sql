...
-- Fix: Remove non-existent columns (graduated_at, graduation_notes) from UPDATE
-- Fix: Correct column names in user_activity_log INSERT (action_type, details, performed_by)
...
BEGIN
  ...
  -- Calculate new roles by removing 'recruit' and adding 'agent'
  ...
  -- Log the activity
  INSERT INTO user_activity_log (user_id, action_type, details, performed_by)
  VALUES (
    p_recruit_id,
    'graduated_to_agent',
    jsonb_build_object(
      'description', 'Graduated to agent with ' || p_contract_level || '% contract level',
...
Preserves existing non-recruit roles during graduation.
