-- Migration: Fix stale event guard to allow re-subscribe after cancellation
--
-- The guard added in 20260225163349 checked for ANY customer.subscription.deleted
-- event for the user. This blocks legitimate re-subscribe flows where a user
-- cancels and later subscribes again with a NEW Stripe subscription ID.
--
-- Fix: Match the deleted event by the SAME stripe_subscription_id (stored in
-- event_data JSONB at data.object.id) so only stale events for the SAME
-- subscription are blocked. New subscriptions with different IDs pass through.
