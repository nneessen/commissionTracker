-- Migration: Fix admin_deleteuser to also delete recruit_invitations
-- Date: 2026-01-29
-- Problem: When a recruit is deleted, their recruit_invitations record persists,
--          causing them to still appear in the recruiting pipeline as "pending"
...
BEGIN
  ...
  SELECT user_id, email INTO v_auth_user_id, v_user_email
  FROM user_profiles
  ...
  -- RECRUIT INVITATIONS - Delete by recruit_id OR by email (for pending invitations)
...
UPDATED: Now also deletes recruit_invitations by recruit_id, email, and inviter_id.';
