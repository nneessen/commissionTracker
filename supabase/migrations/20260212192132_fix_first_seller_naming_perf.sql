-- Fix check_first_seller_naming_unified performance
-- This function was taking 39s+ and killing the DB under concurrent load

-- 1. Add composite index on the two columns the function filters on
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_daily_sales_logs_first_seller_date
ON daily_sales_logs (first_seller_id, log_date);

-- 2. Recreate function with statement_timeout to prevent runaway queries
