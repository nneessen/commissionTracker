-- supabase/migrations/20260211201817_fix_timeline_tenant_isolation_and_negative_days.sql
--
-- Fixes from code review:
-- 1. get_lead_vendor_policy_timeline: add tenant isolation via get_my_imo_id()
--    (was SECURITY DEFINER with no IMO scoping — cross-tenant data exposure)
-- 2. get_lead_vendor_heat_metrics: filter out negative days_to_first_sale
--    (data quality issues could corrupt the median)

-- ============================================================================
-- 1. Fix: get_lead_vendor_policy_timeline — add IMO scoping
-- ============================================================================
...
AS $$
  ...
  WHERE lp.vendor_id = p_vendor_id
    AND lp.vendor_id IN (SELECT id FROM lead_vendors WHERE imo_id = get_my_imo_id())
...
-- ============================================================================
-- 2. Fix: get_lead_vendor_heat_metrics — filter negative days_to_first_sale
-- ============================================================================
...
AS $$
  ...
  pack_first_sale AS (
    SELECT
      rp.vendor_id,
      rp.pack_id,
      GREATEST((MIN(lkp.submit_date)::date - rp.purchase_date::date), 0) as days_to_first_sale
