...
-- Fix: Cast text values to health_class enum type
...
BEGIN
  ...
  LOOP
    ...
    -- Determine health class based on severity
    IF v_condition.severity = 'absolute' THEN
      v_outcome_health := 'decline'::health_class;
    ELSE
      v_outcome_health := 'refer'::health_class;
    END IF;

    -- Create the knockout rule with proper enum casting
...
-- Also fix the age rules function
