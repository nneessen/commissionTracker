...
serve(async (req) => {
  ...
  try {
    ...
    if (isTokenAboutToExpire(integration.token_expires_at, 7)) {
      console.log(
        "[instagram-send-message] Token expiring within 7 days, refreshing proactively",
      );
      ...
      if (
        ...
        console.log(
          "[instagram-send-message] Token refreshed proactively, using new token",
        );
      } else {
        console.warn(
          "[instagram-send-message] Proactive refresh failed, continuing with current token",
        );
      }
    }
    ...
    if (apiData.error) {
      ...
      if (isTokenInvalid) {
        ...
        if (isTokenRecentlyExpired(integration.token_expires_at)) {
          console.log(
            "[instagram-send-message] Token recently expired, attempting refresh",
          );
          ...
          if (
            ...
            if (updated) {
              console.log(
                "[instagram-send-message] Token refreshed, client should retry",
              );
      ...
      if (isRateLimit) {
        console.warn(
          `[instagram-send-message] Rate limited: ${apiData.error.message}`,
        );
        return new Response(
          JSON.stringify({
            ok: false,
      ...
      if (isServerError) {
        console.warn(
          `[instagram-send-message] Meta server error: ${apiData.error.message}`,
        );
        return new Response(
          JSON.stringify({
            ok: false,
