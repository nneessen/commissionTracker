...
import {
  formatInTimeZone,
  toZonedTime,
} from "https://esm.sh/date-fns-tz@3.1.3";
...
function isDigestDue(
  ...
  const currentTotalMinutes =
    userCurrentTime.hour * 60 + userCurrentTime.minute;
  ...
  if (last_digest_sent_at) {
    ...
    const hoursSinceLastDigest =
      (currentTimeUtc.getTime() - lastSent.getTime()) / (1000 * 60 * 60);
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject,
        html,
        tags: [{ name: "type", value: "notification-digest" }],
      }),
    });
...
async function processUserDigest(
  ...
  try {
    ...
    if (!notifications || notifications.length === 0) {
      console.log(
        `[NotificationDigests] No new notifications for ${user.email}`,
      );
      result.success = true;
      return result;
    }
    ...
    result.success = true;
    console.log(
      `[NotificationDigests] Sent digest to ${user.email} with ${notifications.length} notifications`,
    );
  } catch (err) {
    const error = err instanceof Error ? err.message : "Unknown error";
    result.error = error;
...
serve(async (req) => {
  ...
  try {
    ...
    const usersToProcess: UserWithDigestPrefs[] = usersWithDigest
      .map((prefs) => {
        ...
        return {
          ...
          email_digest_timezone:
            prefs.email_digest_timezone || "America/New_York",
    ...
    console.log(
      `[NotificationDigests] Processing ${usersToProcess.length} user(s)`,
    );
    ...
    const totalNotifications = results.reduce(
      (sum, r) => sum + r.notificationCount,
      0,
    );

    console.log(
      `[NotificationDigests] Completed: ${successful} successful, ${failed} failed, ${totalNotifications} total notifications`,
    );
