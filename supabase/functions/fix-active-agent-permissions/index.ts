...
serve(async (req) => {
  ...
  try {
    ...
    if (!roleData || roleError) {
      ...
      if (createError) {
        throw new Error(
          `Failed to create active_agent role: ${createError.message}`,
        );
      }
      roleData = newRole;
    }
    ...
    const { data: permissions, error: permError } = await supabase
      ...
      .select("id, code")
      .or(
        `code.like.nav.%,code.like.policies.%,code.like.commissions.%,code.like.expenses.%,code.like.targets.%,code.like.analytics.%,code.like.reports.%,code.like.hierarchy.%,code.like.email.%`,
      )
      .not("code", "like", "admin.%")
