...
serve(async (req) => {
  ...
  try {
    ...
    if (isTokenAboutToExpire(integration.token_expires_at, 7)) {
      console.log(
        "[instagram-get-messages] Token expiring within 7 days, refreshing proactively",
      );
      ...
      if (
        ...
        console.log(
          "[instagram-get-messages] Token refreshed proactively, using new token",
        );
      } else {
        console.warn(
          "[instagram-get-messages] Proactive refresh failed, continuing with current token",
        );
      }
    }
